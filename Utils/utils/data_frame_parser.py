'''
Author: 成凯阳
Date: 2022-04-15 10:46:36
LastEditors: 成凯阳
LastEditTime: 2022-04-29 08:58:16
FilePath: /Main/Utils/utils/data_frame_parser.py
...
# for linux env.
# sys.path.insert(0,'..')
from Utils.utils.smile_to_graph import GGNNPreprocessor,MolFeatureExtractionError
from Utils.utils.data_loader import NumpyTupleDataset
...
class DataFrameParser(object):
    ...
    def parse(self, voc,df, return_smiles=False, target_index=None,
        ...
        if isinstance(pp, GGNNPreprocessor):
            ...
            for row in tqdm(df.itertuples(index=False), total=df.shape[0]):
                ...
                pred = voc.encode(voc.tokenize(smile))
                # pred=pred.cpu().numpy()
                
                # # pre=pred[:-1].cpu().numpy()
                # pred=numpy.pad(pred, (voc.vocab['<pad>'],24-len(pred)), 'constant')
                pre=pred[:-1]
                next=pred[1:]
                maxl=max(maxl,len(pred))
                # TODO(Nakago): Check.
                ...
                try:
                    ...
                    input_features = pp.get_input_features(mol,pred,next)
            ...
            features[2]=pad_sequence(features[2], batch_first=True, padding_value=voc.vocab['<pad>']).cpu().numpy()
            features[3]=pad_sequence(features[3], batch_first=True, padding_value=voc.vocab['<pad>']).cpu().numpy()
    ...
    def prose(self, df, return_smiles=False, target_index=None,
              return_is_successful=False):
        """parse DataFrame using `preprocessor`
        ...
        if isinstance(pp, GGNNPreprocessor):
            ...
            for row in tqdm(df.itertuples(index=False), total=df.shape[0]):
                ...
                try:
                    ...
                    # we obtain canonical smiles
                    canonical_smiles, mol = pp.prepare_smiles_and_mol(mol)
                    input_features = pp.get_input_feature(mol)
