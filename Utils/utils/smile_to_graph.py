'''
Author: 成凯阳
Date: 2022-04-15 10:46:58
LastEditors: 成凯阳
LastEditTime: 2022-05-08 14:03:42
FilePath: /Main/Utils/utils/smile_to_graph.py
...
class GGNNPreprocessor(object):
    ...
    def get_input_features(self, mol,pred,next):
        """get input features
        ...
        atomc_array = construct_atomic_array(mol, out_size=self.out_size)
        adj_array = construct_discrete_edge_matrix(mol, out_size=self.out_size)
        return atom_array, adj_array,pred,next,atomc_array
    def get_input_feature(self, mol):
        """get input features
...
def construct_atomic_array(mol, out_size=-1):
    """Returns atomic numbers of atoms consisting a molecule.
    ...
    CHIRAL_TAG_DICT = {ct: i+1 for i, ct in enumerate(CHIRAL_TAG)}
