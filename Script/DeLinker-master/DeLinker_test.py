...
def return_csv(generated_all_smiles,path):
    ...
    d = {'diversity':[div],  'unique':[unique_ratio],'nolvety':[val_rate]}
...
class DenseGGNNChemModel(ChemModel):
    def __init__(self, args,input_param):
        super().__init__(args,input_param)
    ...
    def default_params(cls):
        ...
        params.update({
                        ...
                        'number_of_generation_per_valid': 20,
                        'maximum_distance': 50,
                        ...
                        "min_atoms": 4,
                        "max_atoms": 20,
                        "output_name": 'generate.txt',
                        "result_name":'datainfo.csv',
                        "data_d":'tmp/',
                        "restore":'models/pretrained_DeLinker_model.pickle',
                         "generate_file":'1.txt',
                        })
    ...
    def generate_graph_with_state(self, random_normal_states, random_normal_states_in, num_vertices,
        ...
        generated_all_smiles.append( Chem.MolToSmiles(best_mol))
    ...
    def generate_new_graphs(self, data, return_list=False):
        ...
        generated_all_smiles=generated_all_smiles[:le_h]
        ...
        for ind,i in enumerate(generated_all_smiles):
            mol=Chem.MolFromSmiles(i)
            data_di=self.args['data_d']
            lista = [i]
            datass = pd.DataFrame([lista])
            datass.to_csv('./renwuer.csv',mode='a',header=False,index=False)#
            filepath=os.path.join(data_di, '{}.png'.format(ind))
            Draw.MolToFile(mol, filepath, size=(600,600))
        a=return_csv(generated_all_smiles,path=self.args['result_name'])
